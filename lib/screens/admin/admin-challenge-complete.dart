...
class AdminChallangeComplete extends StatefulWidget {
  @override
  _AdminChallangeCompleteState createState() => _AdminChallangeCompleteState();
}

class _AdminChallangeCompleteState extends State<AdminChallangeComplete> {
  ...
  filterData(){
    setState(() {
      if(search.text==''&&location=='all'&&dateString==null){
        //nothing filter
        getDetails(collectionReference.orderBy("timestamp", descending: true).where('completed', isEqualTo: true));
        print('nothing');
      }
      else if(search.text==''&&location=='all'&&dateString!=null){
        //date only
        getDetails(collectionReference.where('completed', isEqualTo: true).where('date', isEqualTo: dateString));
        print('date');
      }
      else if(search.text==''&&location!='all'&&dateString==null){
        //location
        getDetails(collectionReference.orderBy("timestamp", descending: true).where('completed', isEqualTo: true).where('location',isEqualTo: location));
        print('location');
      }
      else if(search.text==''&&location!='all'&&dateString!=null){
        //location and date
        getDetails(collectionReference.where('completed', isEqualTo: true).where('location',isEqualTo: location).where('date', isEqualTo: dateString));
        print('location and date');
      }
      else if(search.text!=''&&location=='all'&&dateString==null){
        //search
        getDetails(collectionReference.orderBy("timestamp", descending: true).where('completed', isEqualTo: true).where('search',arrayContains: search.text[0].toUpperCase()+search.text.substring(1)));
        print('search');
      }
      else if(search.text!=''&&location=='all'&&dateString!=null){
        //search and date
        getDetails(collectionReference.where('completed', isEqualTo: true).where('search',arrayContains: search.text[0].toUpperCase()+search.text.substring(1)).where('date', isEqualTo: dateString));
        print('search and date');
      }
      else if(search.text!=''&&location!='all'&&dateString==null){
        //search and location
        getDetails(collectionReference.orderBy("timestamp", descending: true).where('completed', isEqualTo: true).where('search',arrayContains: search.text[0].toUpperCase()+search.text.substring(1)).where('location',isEqualTo: location));
        print('search and location');
      }
      else if(search.text!=''&&location!='all'&&dateString!=null){
        //all
        getDetails(collectionReference.where('completed', isEqualTo: true).where('search',arrayContains: search.text[0].toUpperCase()+search.text.substring(1)).where('location',isEqualTo: location).where('date', isEqualTo: dateString));
        print('all');
      }
    });
